...
class Camera
{
  Camera(Vec3 pos, float theta, float phi)
  {
    position      = new PVector( pos.x, pos.y, pos.z); // initial position
    this.theta    = theta; // rotation around Y axis. Starts with forward direction as ( 0, 0, -1 )
    this.phi      = phi;//; // rotation around X axis. Starts with up direction as ( 0, 1, 0 )
    moveSpeed     = 50;
